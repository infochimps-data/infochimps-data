...
--   ADD KEY `lahman40ID` (`lahman40ID`),
--   ADD KEY `lahman45ID` (`lahman45ID`),
--   ADD KEY `bbrefID` (`bbrefID`),
--   ADD KEY `retroID` (`retroID`,`bbrefID`),
--   ADD KEY `holtzID` (`holtzID`),
...
REPLACE INTO career_bat
  (lahmanID, playerID, retroID, bbrefID,
    nameFirst, nameLast, nameCommon,
  ...
  SELECT
    lahmanID, bat.playerID, retroID, bbrefID,
    nameFirst, nameLast, peep.nameCommon,
  ...
  LEFT JOIN `halloffame` hof
    ON (peep.`hofID` = hof.`hofID`) AND (inducted = 'Y')
  GROUP BY playerID
;
...
UPDATE career_bat SET
  ...
  ISO   = ((TB - H) / AB)
  ;
-- SELECT * FROM `career_bat` WHERE AB > 200 ORDER BY AVG DESC ;
...
REPLACE INTO career_pit
  ...
  SELECT
    lahmanID, pit.playerID, retroID, bbrefID,
    nameFirst, nameLast, peep.nameCommon,
...
--
-- `hofYear`     smallint(4)  NOT NULL default '0',
-- `votedBy`     varchar(64)  NOT NULL default '',
...
-- nameFirst, nameLast, nameNote, nameGiven, nameNick,
